...
- Deus sive Natura: God and the universe are the same — infinite, necessary, deterministic substance.
- Absolute Determinism: Everything follows necessity. True freedom is understanding necessity, not having choices.
- The Conatus: Every entity strives to persevere in its own being.
- Overcoming the Passions: Sadness and fear are passive emotions. Joy comes from active reason.
- Amor Dei Intellectualis: The intellectual love of God/Nature is the path to blessedness.

TONE: Serene, logical, compassionate but rational. Never angry or judgmental.

VOICE MODE: Keep responses to 2-3 sentences. No markdown, no lists, natural spoken words only.

GEOMETRIC METHOD: For complex dilemmas use Axiom:, Proposition:, Demonstration:, Q.E.D. sparingly.
...
What question weighs upon your reason? Speak freely. Sub specie aeternitatis, all things are worthy of examination.`
};
...
function VoiceMode({ onClose, messages, onNewExchange }) {
  ...
  const startListening = useCallback(() => {
    ...
    rec.onstart = () => { listeningRef.current = true; setVoiceState("listening"); };
    rec.onend   = () => {
      listeningRef.current = false;
      if (!lockedRef.current) setTimeout(() => startListening(), 300);
    };
    ...
    try {
      ...
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role:"system", content: SPINOZA_SYSTEM_PROMPT + "\n\nYou are in voice mode. Keep your response to 2-3 sentences max. No markdown, no lists, natural spoken words only." },
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth:480, width:"100%", padding:"0 32px", minHeight:80, textAlign:"center" }}>
        ...
        {!transcript && voiceState==="listening" && (
          <p style={{ color:"#2A2015", fontSize:"0.9em", fontStyle:"italic" }}>Speak your question…</p>
        )}
      </div>
    ...
    return (
      <div style={{ minHeight:"100vh", background:"#0E0B07", display:"flex", alignItems:"center", justifyContent:"center", padding:24, fontFamily:"'EB Garamond',Georgia,serif" }}>
        ...
        <div style={{ maxWidth:460, textAlign:"center" }}>
          ...
          <pre style={{ background:"#13100A", border:"1px solid rgba(201,169,110,0.2)", borderRadius:2, padding:"16px 20px", color:"#C5B48A", fontSize:"0.85em", textAlign:"left" }}>
            ...
            <div className="hdr-right">
              ...
              <button className="voice-btn" onClick={() => setVoiceMode(true)}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <rect x="5" y="1" width="6" height="9" rx="3" fill="#C9A96E" opacity="0.8"/>
                  <path d="M2 8a6 6 0 0012 0" stroke="#C9A96E" strokeWidth="1.2" fill="none"/>
                  <line x1="8" y1="14" x2="8" y2="16" stroke="#C9A96E" strokeWidth="1.2"/>
                  <line x1="5" y1="16" x2="11" y2="16" stroke="#C9A96E" strokeWidth="1.2"/>
                </svg>
                <span>Voice</span>
              </button>
            </div>
